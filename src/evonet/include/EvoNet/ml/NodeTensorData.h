...
#ifndef EVONET_NODETENSORDATA_H
#define EVONET_NODETENSORDATA_H
...
public:
    ...
    std::string getLayerIntegration() const { return layer_integration_; }
		...
		Eigen::TensorMap<Eigen::Tensor<TensorT, 3>> getInput() { Eigen::TensorMap<Eigen::Tensor<TensorT, 3>> input(h_input_.get(), batch_size_, memory_size_, layer_size_); return input; }; ///< input copy getter
...
//CEREAL_REGISTER_TYPE(EvoNet::NodeTensorDataCpu<float>);
//// TODO: add double, int, etc.
//#if COMPILE_WITH_CUDA
//CEREAL_REGISTER_TYPE(EvoNet::NodeTensorDataGpu<float>);
//// TODO: add double, int, etc.
//#endif

#endif //EVONET_NODETENSORDATA_H