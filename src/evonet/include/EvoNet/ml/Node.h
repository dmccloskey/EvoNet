...
#ifndef EVONET_NODE_H
#define EVONET_NODE_H

// .h
#include <EvoNet/ml/ActivationFunction.h>
#include <EvoNet/ml/IntegrationFunction.h>
...
public:
    ...
    Node(const std::string& name, const EvoNet::NodeType& type, const EvoNet::NodeStatus& status, 
    ...
    void setType(const EvoNet::NodeType& type); ///< type setter
    EvoNet::NodeType getType() const; ///< type getter

    void setStatus(const EvoNet::NodeStatus& status); ///< status setter
    EvoNet::NodeStatus getStatus() const; ///< status getter
...
private:
    ...
    std::string name_ = ""; ///< Node Name
		...
		EvoNet::NodeType type_; ///< Node Type
    EvoNet::NodeStatus status_; ///< Node Status   
	...
	template<typename TensorT>
	Node<TensorT>::Node(const std::string& name, const EvoNet::NodeType& type, const EvoNet::NodeStatus& status,
		const std::shared_ptr<ActivationOp<TensorT>>& activation, const std::shared_ptr<ActivationOp<TensorT>>& activation_grad,
	...
	template<typename TensorT>
	void Node<TensorT>::setType(const EvoNet::NodeType& type)
	{
		type_ = type;
	}
	template<typename TensorT>
	EvoNet::NodeType Node<TensorT>::getType() const
	{
		return type_;
	}

	template<typename TensorT>
	void Node<TensorT>::setStatus(const EvoNet::NodeStatus& status)
	{
		status_ = status;
	}
	template<typename TensorT>
	EvoNet::NodeStatus Node<TensorT>::getStatus() const
	{
		return status_;
	}
