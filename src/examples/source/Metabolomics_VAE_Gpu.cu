  ...
  void makeModelFCVAE_1(Model<TensorT>& model, const int& n_inputs, const int& n_outputs, const int& n_encodings, const bool& linear_scale_input, const bool& log_transform_input, const bool& standardize_input, const bool& add_norm = false,
    ...
    if (n_en_hidden_0 > 0) {
      node_names = model_builder.addFullyConnected(model, "EN0", "EN0", node_names, n_en_hidden_0,
        ...
        std::make_shared<AdamOp<TensorT>>(AdamOp<TensorT>(5e-5, 0.9, 0.999, 1e-8, 1e2)), 0.0f, 0.0f, false, true);
      if (add_norm) {
        ...
        node_names = model_builder.addSinglyConnected(model, "EN0-Norm-gain", "EN0-Norm-gain", node_names, node_names.size(),
          ...
          std::make_shared<AdamOp<TensorT>>(AdamOp<TensorT>(5e-5, 0.9, 0.999, 1e-8, 1e2)),
          0.0, 0.0, true, true);
      }
    }
    if (n_en_hidden_1 > 0) {
...
void main_reconstruction(const std::string& biochem_rxns_filename,
  ...
  const bool& use_fold_change = false)
{
  ...
  if (make_data_caches) {
    ...
    metabolomics_data.makeTrainingDataCache(input_data_training, model_trainer.getNEpochsEvaluation(), model_trainer.getBatchSize(), model_trainer.getMemorySize(),
    ...
    metabolomics_data.makeValidationDataCache(input_data_validation, model_trainer.getNEpochsEvaluation(), model_trainer.getBatchSize(), model_trainer.getMemorySize(),
      input_nodes.size(), n_loss_output_nodes, n_metric_output_nodes);
  ...
  model_trainer.setLossFunctions({
    ...
    std::make_shared<KLDivergenceMuLossOp<float>>(KLDivergenceMuLossOp<float>(1e-6, 0.0)), //FIXME
    std::make_shared<KLDivergenceLogVarLossOp<float>>(KLDivergenceLogVarLossOp<float>(1e-6, 0.0))
    });
  model_trainer.setLossFunctionGrads({
    std::shared_ptr<LossFunctionGradOp<float>>(new MAPELossGradOp<float>(1e-6, 1.0)),
    std::make_shared<KLDivergenceMuLossGradOp<float>>(KLDivergenceMuLossGradOp<float>(1e-6, 0.0)),
    std::make_shared<KLDivergenceLogVarLossGradOp<float>>(KLDivergenceLogVarLossGradOp<float>(1e-6, 0.0))
    });
