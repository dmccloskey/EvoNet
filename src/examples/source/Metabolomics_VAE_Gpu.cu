  ...
  bool make_model = true, bool simulate_MARs = true, bool sample_concs = true)
{
  ...
  model_trainer.setLossFunctionGrads({
    ...
    std::shared_ptr<LossFunctionGradOp<float>>(new KLDivergenceMuGradOp<float>(1e-6, 30.0)),
    std::shared_ptr<LossFunctionGradOp<float>>(new KLDivergenceLogVarGradOp<float>(1e-6, 30.0)) });
