...
void convertHashTensorToStrTensor(std::size_t* hash_data, std::string* string_data, const std::unordered_map<std::size_t, std::string>& hash_table, const int& dim_sizes...) {
  Eigen::TensorMap<Eigen::Tensor<std::size_t, 1>> hash_tensor(hash_data, dim_sizes);
  Eigen::TensorMap<Eigen::Tensor<std::string, 1>> string_tensor(string_data, dim_sizes);
...
void stringCompareGpuEx(const std::size_t& dim1) {
  ...
  in1_str(1) = "He";
  in1_str(2) = "hello";
  convertStrTensorToHashTensor(h_in1, h_str_in1, cpuDevice, (int)dim1);
