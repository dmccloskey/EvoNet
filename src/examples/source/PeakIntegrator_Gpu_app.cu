...
public:
  ...
  */
  ...
  void makeCovNet(Model<TensorT>& model, const int& n_inputs, const int& n_outputs, int n_depth_1 = 32, int n_depth_2 = 2, int n_fc = 128, bool add_norm = false, bool specify_layers = false) {
    ...
    // Add the first convolution -> max pool -> ELU layers
    ...
    // Add the second convolution -> max pool -> ELU layers
...
void main_DenoisingAE(const bool& make_model, const bool& train_model) {
  ...
  if (make_model) {
    ...
    //model_trainer.makeCovNet(model, input_nodes.size(), output_nodes_intensity.size(), 12, 2, 128, false, true);  // Minimal solving model
  }
  else {
