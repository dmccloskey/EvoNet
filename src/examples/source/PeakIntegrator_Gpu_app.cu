...
public:
	...
	*/
	...
	void makeCompactCovNetAE(Model<TensorT>& model, const int& n_inputs, const int& n_outputs, int n_encodings = 32,
		...
		if (add_scalar) {
			node_names_conv1 = model_builder.addScalar(model, "EncScalar1", "EncScalar1", node_names_conv1, 5 * node_names_conv0.size(),
				std::shared_ptr<ActivationOp<TensorT>>(new LinearOp<TensorT>()),
				std::shared_ptr<ActivationOp<TensorT>>(new LinearGradOp<TensorT>()),
				true);
		}
...
// define the model trainers and resources for the trainers
	...
	if (make_model) {
		...
		model_trainer.makeCompactCovNetAE(model, input_size, input_size, encoding_size, 12, 12, true);
	}
	else {
