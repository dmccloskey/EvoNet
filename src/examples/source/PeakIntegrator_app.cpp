...
void main_DenoisingAE(const std::string& data_dir, const bool& make_model, const bool& train_model) {
  ...
  std::vector<ModelInterpreterDefaultDevice<double>> model_interpreters;
  for (size_t i = 0; i < n_threads; ++i) {
    ModelResources model_resources = { ModelDevice(0, 1) };
    ModelInterpreterDefaultDevice<double> model_interpreter(model_resources);
  ...
  if (make_model) {
    ...
    ModelInterpreterFileDefaultDevice<double> model_interpreter_file;
