...
public:
  ...
  void simulateTrainingData(Eigen::Tensor<TensorT, 3>& input_data, Eigen::Tensor<TensorT, 3>& loss_output_data, Eigen::Tensor<TensorT, 3>& metric_output_data, Eigen::Tensor<TensorT, 2>& time_steps) override
  {
    ...
    for (int batch_iter = 0; batch_iter < batch_size; ++batch_iter) {
      for (int memory_iter = 0; memory_iter < memory_size; ++memory_iter) {
        ...
        for (int nodes_iter = 0; nodes_iter < n_loss_output_nodes; ++nodes_iter) {
          loss_output_data(batch_iter, memory_iter, nodes_iter) = 0.0f; // TODO;
        }

        for (int nodes_iter = 0; nodes_iter < n_metric_output_nodes; ++nodes_iter) {
          metric_output_data(batch_iter, memory_iter, nodes_iter) = 0.0f; // TODO;
        }
      }
    }
  ...
  void simulateEvaluationData(Eigen::Tensor<TensorT, 3>& input_data, Eigen::Tensor<TensorT, 3>& metric_output_data, Eigen::Tensor<TensorT, 2>& time_steps) override
  {
    ...
    const int memory_size = input_data.dimension(1);
    const int n_input_nodes = input_data.dimension(2);
    const int n_output_nodes = metric_output_data.dimension(2);
...
BOOST_AUTO_TEST_CASE(simulateTrainingData3)
{
  DataSimulatorExt<float> datasimulator;

  Eigen::Tensor<float, 3> input_data(1, 1, 1);
  Eigen::Tensor<float, 3> loss_output_data(1, 1, 1);
  Eigen::Tensor<float, 3> metric_output_data(1, 1, 1);
  Eigen::Tensor<float, 2> time_steps(1, 1);

  datasimulator.simulateTrainingData(input_data, loss_output_data, metric_output_data, time_steps);

  BOOST_CHECK_EQUAL(input_data(0, 0, 0), 0.0f);
  BOOST_CHECK_EQUAL(loss_output_data(0, 0, 0), 0.0f);
  BOOST_CHECK_EQUAL(metric_output_data(0, 0, 0), 0.0f);
  BOOST_CHECK_EQUAL(time_steps(0, 0), 1.0f);
}
...
BOOST_AUTO_TEST_CASE(simulateValidationData3)
{
  DataSimulatorExt<float> datasimulator;
  ...
  datasimulator.simulateValidationData(input_data, loss_output_data, metric_output_data, time_steps);
...
BOOST_AUTO_TEST_CASE(simulateEvaluationData1)
{
	DataSimulatorExt<float> datasimulator;
...
BOOST_AUTO_TEST_CASE(simulateEvaluationData2)
{
  DataSimulatorExt<float> datasimulator;
  ...
  datasimulator.simulateEvaluationData(input_data, metric_output_data, time_steps);
