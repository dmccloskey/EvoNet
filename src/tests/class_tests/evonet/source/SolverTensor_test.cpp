...
#include <EvoNet/ml/SolverTensor.h>
...
BOOST_AUTO_TEST_CASE(settersAndGetters) 
{
  ...
  BOOST_CHECK_CLOSE(operation.getEps(), 1e-24, 1e4);
  ...
  BOOST_CHECK_CLOSE(ssd_op.getEps(), 1e-24, 1e4);
  ...
  BOOST_CHECK_CLOSE(adam_op.getEps(), 1e-24, 1e4);
  ...
  SVAGTensorOp<float, Eigen::DefaultDevice> svag_op(10.0f, 1.0f, 0.55f);
  BOOST_CHECK_EQUAL(svag_op.getName(), "SVAGTensorOp");
  BOOST_CHECK_CLOSE(svag_op.getGradientThreshold(), 10.0, 1e4);
  BOOST_CHECK_CLOSE(svag_op.getGradientNoiseSigma(), 1.0, 1e4);
  BOOST_CHECK_CLOSE(svag_op.getGradientNoiseGamma(), 0.55, 1e4);
  BOOST_CHECK_CLOSE(svag_op.getEps(), 1e-24, 1e4);
  //BOOST_CHECK_EQUAL(svag_op.getParameters(), "gradient_threshold:1000000.000000;gradient_noise_sigma:1.000000;gradient_noise_gamma:0.550000;learning_rate:0.010000;momentum:0.900000;momentum2:0.999000;delta:0.000000;momentum_prev:0.000000;momentum2_prev:0.000000");
  ...
  BOOST_CHECK_CLOSE(dummy_solver_op.getEps(), 1e-24, 1e4);
	//BOOST_CHECK_EQUAL(dummy_solver_op.getParameters(), "");
}

BOOST_AUTO_TEST_CASE(annealGradientNoiseSigma)
{
  SGDTensorOp<float, Eigen::DefaultDevice> operation(10.0f, 1.0f, 0.55f);
  BOOST_CHECK_CLOSE(operation.annealGradientNoiseSigma(1), 0.683020115, 1e-4);
  BOOST_CHECK_CLOSE(operation.annealGradientNoiseSigma(100), 0.0789992958, 1e-4);
}
...
BOOST_AUTO_TEST_CASE(operationfunctionAdamOp)
{
  ...
  // Test operator call
	operation(weights.data(), errors.data(), solver_params.data(), source_layer_size, sink_layer_size, iter, device);
	BOOST_CHECK_CLOSE(weights(0, 0), 0.99000001, 1e-4);
  ...
  operation(weights1.data(), errors.data(), solver_params1.data(), source_layer_size, sink_layer_size, iter, device);
  BOOST_CHECK_CLOSE(weights1(0, 0), 0.99000001, 1e-4);
  BOOST_CHECK_CLOSE(weights1(1, 0), 0.99000001, 1e-4);
  BOOST_CHECK_CLOSE(errors(0, 0), 0.1, 1e-4);
  BOOST_CHECK_CLOSE(errors(1, 0), 10.0, 1e-4);
  BOOST_CHECK_CLOSE(solver_params1(0, 0, 0), 0.01, 1e-4);
  ...
  operation(weights.data(), errors.data(), solver_params.data(), source_layer_size, sink_layer_size, iter, device);
  BOOST_CHECK(weights(0, 0) != 0.99000001, 1e-4);
...
BOOST_AUTO_TEST_CASE(operationfunctionSVAGOp)
{
  ...
  Eigen::Tensor<float, 2> errors(source_layer_size, sink_layer_size);
  errors.setValues({ {0.1},	{10} });
  Eigen::Tensor<float, 3> solver_params(source_layer_size, sink_layer_size, 4);
  solver_params.setValues({ {{0.01, 0.9, 0.0, 0.0}},
    {{0.01, 0.9, 0.0, 0.0}} });
  ...
  Eigen::Tensor<float, 2> weights1(source_layer_size, sink_layer_size);
  weights1.setConstant(1);
  Eigen::Tensor<float, 3> solver_params1(source_layer_size, sink_layer_size, 4);
  solver_params1.setValues({ {{0.01, 0.9, 0.0, 0.0}},
    {{0.01, 0.9, 0.0, 0.0}} });
  operation.setGradientThreshold(1.0);
