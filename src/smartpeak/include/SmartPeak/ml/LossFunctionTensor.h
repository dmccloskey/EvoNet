...
public: 
    ...
    ~EuclideanDistanceGradTensorOp(){};
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			error_tensor.chip(time_step, 1).device(device) += ((expected_tensor - predicted_chip) / ((expected_tensor - predicted_chip - expected_tensor.constant(this->eps_)).pow(2).sqrt()));
		};
  };
...
public: 
    ...
    ~L2NormTensorOp(){};
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			Eigen::TensorMap < Eigen::Tensor<TensorT, 2>> error_tensor(error, batch_size, memory_size);
			auto predicted_chip = predicted_tensor.chip(time_step, 1);
			error_tensor.chip(time_step, 1).device(device) += ((expected_tensor - (predicted_chip).pow(2)) * expected_tensor.constant((TensorT)0.5)).sum(Eigen::array<int, 1>({ 1 })).clip(-1e9, 1e9); // modified to simplify the derivative
		};
  };
    ...
    ~BCETensorOp(){};
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			error_tensor.chip(time_step, 1).device(device) += (-(
				expected_tensor * (predicted_chip + expected_tensor.constant(this->eps_)).log() + // check if .clip(1e-6, 1) should be used instead
				(expected_tensor.constant((TensorT)1) - expected_tensor) * (expected_tensor.constant((TensorT)1) - (predicted_chip - expected_tensor.constant(this->eps_))).log())).sum(Eigen::array<int, 1>({ 1 })).clip(-1e9, 1e9);
		};
  };
...
public: 
    ...
    ~BCEGradTensorOp(){};
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			error_tensor.chip(time_step, 1).device(device) += (-(predicted_chip - expected_tensor) / (((predicted_chip - expected_tensor.constant((TensorT)1)) * predicted_chip) - expected_tensor.constant(this->eps_))).clip(-1e9, 1e9);
...
public: 
    ...
    ~NegativeLogLikelihoodTensorOp() = default;
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			error_tensor.chip(time_step, 1).device(device) += ((-expected_tensor * (predicted_chip.clip(1e-6, 1).log())) * expected_tensor.constant(1 / (TensorT)layer_size)).sum(Eigen::array<int, 1>({ 1 }));
		};
	private:
...
public: 
    ...
    ~NegativeLogLikelihoodGradTensorOp(){};
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			error_tensor.chip(time_step, 1).device(device) += (expected_tensor / (predicted_chip + expected_tensor.constant(this->eps_)) / expected_tensor.constant((TensorT)layer_size)).clip(-1e9, 1e9);
		};
	private:
		TensorT n_ = 1.0; ///< the number of total classifiers
  };
	...
	*/
	...
	public:
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			//error_tensor.chip(time_step, 1).device(device) += (-expected_tensor.constant((TensorT)0.5) + expected_tensor.constant((TensorT)0.5)*predicted_chip.pow(2)).sum(Eigen::array<int, 1>({ 1 })).clip(-1e9, 1e9);
			error_tensor.chip(time_step, 1).device(device) += expected_tensor.constant((TensorT)0); // testing...
		};
	};
	...
	*/
	...
	public:
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			//error_tensor.chip(time_step, 1).device(device) += expected_tensor.constant((TensorT)2) * predicted_chip;
			error_tensor.chip(time_step, 1).device(device) += expected_tensor.constant((TensorT)0); // testing...
		};
	};
	...
	*/
	...
	public:
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			//error_tensor.chip(time_step, 1).device(device) += (-expected_tensor.constant((TensorT)0.5) - expected_tensor.constant((TensorT)0.5)*predicted_chip + (expected_tensor.constant((TensorT)0.5)*predicted_chip).exp()).sum(Eigen::array<int, 1>({ 1 })).clip(-1e9, 1e9);
			error_tensor.chip(time_step, 1).device(device) += expected_tensor.constant((TensorT)0); // testing...
		};
	};
	...
	*/
	...
	public:
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			error_tensor.chip(time_step, 1).device(device) += -((expected_tensor - expected_tensor.constant((TensorT)1))*predicted_chip.exp() + expected_tensor)/(predicted_chip.exp() + expected_tensor.constant((TensorT)1));
		};
	};
	...
	*/
	...
	public:
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			error_tensor.chip(time_step, 1).device(device) += ((-expected_tensor * (stable_softmax.clip(1e-6, 1).log())) * expected_tensor.constant(1 / (TensorT)layer_size)).sum(Eigen::array<int, 1>({ 1 }));
		};
	};
	...
	*/
	...
	public:
		...
		void operator()(TensorT* predicted, TensorT* expected, TensorT* error, const int& batch_size, const int& memory_size, const int& layer_size, const int& time_step, DeviceT& device) const
		{
			...
			error_tensor.chip(time_step, 1).device(device) += ((predicted_chip - expected_tensor) / expected_tensor.constant((TensorT)layer_size)).clip(-1e9, 1e9);
		};
	};
